...
#include <JC_Button.h>            //http://github.com/JChristensen/Button   (Make sure to use version 1.0 for compatibility)
...
#include <DallasTemperature.h>    //https://www.arduino.cc/reference/en/libraries/dallastemperature/
#include <OneWire.h>              
#include <DS18B20.h>              //https://www.arduino.cc/reference/en/libraries/ds18b20/
...
#define LCD_D7 12
#define RTC_POWER 3
#define DS18B20 6                 //and/or external sensor DS18B20
...
#define OUTAGE_LENGTH 10           //8 data bytes for each outage (start and end timestamps, both are time_t values)
#define MAX_OUTAGES 3             //maximum number of outage timestamp pairs that can be stored in SRAM. Originally 6, changed by benji
...
void setup()
{
    ...
    RTC.idRead(rtcID.b);
    lcd.setCursor(0, 1);
    for (uint8_t i=0; i<8; i++) lcd << (rtcID.b[i] < 16 ? "0" : "" ) << _HEX(rtcID.b[i]);  //Benjireplaced : w/ x
    delay(MSG_DELAY);
    while (btnSet.read());    //user can hold the message by holding the set button
    lcd.clear();

    nOutage = logOutage();    //log an outage if one occurred
    printToRaspi();           //send serial data to raspberry pi
}
...
//takes the epoch times and maximum temperature from the first outage recorded and prints them onto a single serial line to be read by the python program
void printToRaspi()
{
  time_t powerDown, powerUp;
  uint8_t OTEMP;
  
        powerDown = (*tz).toLocal(read32(FIRST_OUTAGE_ADDR));
        powerUp = (*tz).toLocal(read32(FIRST_OUTAGE_ADDR + 4));
        OTEMP = read16(FIRST_OUTAGE_ADDR + 8);
        
        unsigned long time1 = (unsigned long) powerDown;
        unsigned long time2 = (unsigned long) powerUp;
        int temp = OTEMP;
...
uint8_t displayOutage(int8_t outageNbr)
{
    ...
    if (nOutage == 0) {
        ...
        lcd.setCursor(0, 1);
        lcd.print("U ");        //if all fails, use  lcd << F("  UP ")       
